...
class ModProduct extends CI_Model
{
    ...
    public function getItems($id_principal = null)
    {
        ...
        if ($id_principal) {
            $this->db->where('p.id_principal', $id_principal);
        }
        $this->db
        		->select('p.*, p.name as principal_name, pr.*, pr.name as product_name, pc.category, pu.unit, pu.value, pm.buy_price')
                ->from('principal p')
                ...
                ->join('(SELECT MAX(id_price_movement) max_id, buy_price FROM product_price_movement GROUP BY  id_principal) as pm','pm.id_product = pr.id_product')
